...
def get_supabase_credentials():
    ...
    if not param_prefix:
        return {'url': os.environ['SUPABASE_URL'], 'secret': os.environ['SUPABASE_SECRET']}

    ssm = boto3.client('ssm')
    response = ssm.get_parameters(
        Names=[f'{param_prefix}/url', f'{param_prefix}/secret'], WithDecryption=True
    )
...
class SupabaseUploader:
    ...
    def upload_game_data(self, game_object: dict):
        ...
        logger.info(
            f'Uploading game to Supabase: game_id={game_id}, partition_date={partition_date}, table={self.table_name}'
        )
        ...
        payload = {'game_id': game_id, 'partition_date': partition_date, 'event': game_object}
